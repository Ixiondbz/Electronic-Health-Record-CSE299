...
class App extends Component {
  ...
  render() {
    ...
    return (
      <div className="App">
        <NavbarComp
          isLogged={this.state.loggedAcc}
          onlogout={() => this.setState({ loggedAcc: null, loggedas: null })}
        />
        ...
        {!this.state.loggedAcc ? (
          <DocLogin
            onlogin={(loggedAcc, loggedas) =>
              this.setState({ loggedAcc, loggedas })
            }
            state={this.state}
          />
        ) : this.state.loggedas == 0 ? (
          <Doctor contract={this.state.contract} Acc={this.state.accounts} />
        ) : (
          <Patient contract={this.state.contract} Acc={this.state.accounts} />
        )}
      </div>
    )
  }
